...
dofs = N^2;
LSdofs = 3*dofs;
u = @(x,y) exp(x)*sin(3*pi*y); % Analytical solution
B = @(x,y,u) alpha*[x*u;1]; %Vector Field
f = @(x,y) exp(x)*(mu*9*pi^2-mu+sigma)*sin(3*pi*y)...
    +exp(x)*alpha*[x*u(x,y) 1]*[sin(pi*y) ; pi*cos(pi*y)] + sigma*u(x,y); % Loading function
