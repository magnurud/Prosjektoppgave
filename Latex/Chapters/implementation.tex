...
Where each block $A_{m,n}$ corresponds to  calculating element $m,n$ in the matrix\eref{mat:basicPoisson} for all combinations of $I,J$. In order to keep track of the indexing we use the expressions $I = i+jN$ and $J = k+lN$ where the helping indices denotes the position in $x,y$-coordinates. Let us take a closer look at element $A_{1,3}$ in order to achieve a more compact notation. 
\begin{align}
...
In addition it will be added some extra terms dependent on $\mathbf{b}$ to the upper left $2 \times 2$ block matrix. Let $B_1$ and $B_2$ be diagonal $n^2 \times n^2$ matrices with the values of the first and second component of $\mathbf{b}$ evaluated in each spacial node along the diagonal.  
\begin{align}
...
We can now define the total matrix for the diffusion transport problem $K^{LS}$ as
