...
$\vec{w} = [w_1 \; w_2 ] $ & Negative gradient of u \\
$\mathbf{u} = [ \vec{w} \; u ]^T$ & Solution to the first order transfomation \\ 
$ R_g $ & Lifting function \\
$\tilde{\mathbf{u}} = \mathbf{u}-R_g $ & Solution to the first order transfomation minus the lifting function \\ 
$\vec{b}$ & Two dimensional vector field \\
$f$ & loading function\\
$\vec{f} = (0,0,f)$ & loading function for the first order transformation\\
...
\textbf{Linear Algebra} & A good idea to denote matrices with an underscore ?  \\
$A$ & The stiffness matrix \\
$G$ & The gradient matrix \\
$F$ & The vector obtained from the linear functional $F$ \\ 
